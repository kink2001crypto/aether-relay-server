...
class ChatPanelProvider {
    ...
    async _loadHistory() {
        ...
        if (!workspaceFolder)
            return;
        try {
            const response = await fetch(`${this._serverUrl}/api/chat/history?projectPath=${encodeURIComponent(workspaceFolder.uri.fsPath)}`, { headers: { 'ngrok-skip-browser-warning': 'true' } });
            if (response.ok) {
                const data = await response.json();
    ...
    async _applyCode(code, filePath) {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
    ...
    async _loadConversationByPath(projectPath) {
        try {
            const response = await fetch(`${this._serverUrl}/api/chat/history?projectPath=${encodeURIComponent(projectPath)}`, { headers: { 'ngrok-skip-browser-warning': 'true' } });
            if (response.ok) {
                const data = await response.json();
                this._messages = data.messages || [];
                this._view?.webview.postMessage({
    ...
    async _deleteConversation(projectPath) {
        try {
    ...
    async _sendModelVariants(model) {
        // Model variants
        const variants = {
            gemini: [
    ...
    _getHtmlContent(webview) {
        const cssUri = webview.asWebviewUri(vscode.Uri.file(path.join(this._context.extensionPath, 'resources', 'chat.css')));
        const jsUri = webview.asWebviewUri(vscode.Uri.file(path.join(this._context.extensionPath, 'resources', 'chat.js')));
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
ChatPanelProvider.viewType = 'aether.chatPanel';
