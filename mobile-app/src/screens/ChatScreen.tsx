/**
 * 💬 AETHER Mobile - Chat Screen
 * AI conversation with code blocks and agent actions
 */
...
import { useApp, Message, CodeBlock, AgentAction } from '../hooks/useApp';
...
const TERMINAL_LANGUAGES = ['bash', 'shell', 'sh', 'zsh', 'terminal', 'console', 'cmd', 'powershell'];
...
const LANGUAGE_COLORS: Record<string, [string, string]> = {
...
const isTerminalCommand = (lang: string): boolean => {
  return TERMINAL_LANGUAGES.includes(lang.toLowerCase());
};
...
export default function ChatScreen() {
  const {
    ...
    undoLastChange, lastApplied, executeAction
  } = useApp();
  ...
  const [expandedCode, setExpandedCode] = useState<CodeBlock | null>(null);
  ...
  const handleApplyCode = useCallback((code: string, language: string) => {
    if (!currentProject) {
      Alert.alert('No Project', 'Select a project first to save code');
      return;
    }
  ...
  const handleRunCommand = useCallback((command: string) => {
    ...
    Alert.alert('Run Command?', command.trim(), [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Run', onPress: () => executeCommand(command.trim()) }
    ]);
  }, [currentProject, executeCommand]);

  const handleDelete = useCallback((content: string) => {
    if (!currentProject) {
      Alert.alert('Error', 'Select a project first');
      return;
    }
    ...
    Alert.alert('Delete Files?', `Delete:\n${paths.join('\n')}`, [
      { text: 'Cancel', style: 'cancel' },
      {
  ...
  }, [currentProject, deleteFile, deleteFolder]);

  const handlePreview = useCallback((code: string, language: string) => {
    if (!currentProject) {
    ...
    const timestamp = Date.now().toString().slice(-6);
    const filename = `code_${timestamp}${ext}`;
    setDiffData({ filePath: filename, oldContent: null, newContent: code });
    setShowDiffModal(true);
  }, [currentProject]);

  const handleExecuteAction = useCallback((action: AgentAction) => {
    Alert.alert(
      `Execute ${action.type.replace('_', ' ')}?`,
      action.description,
      [
        { text: 'Cancel', style: 'cancel' },
  ...
  const renderCodeBlock = useCallback((block: CodeBlock, index: number) => {
    const langColors = LANGUAGE_COLORS[block.language.toLowerCase()] || [colors.primary, colors.primaryDark];
    ...
    const labelText = isDelete ? 'DELETE' : isTerminal ? 'TERMINAL' : block.language.toUpperCase();

    return (
      <View key={index} style={[styles.codeBlock, { backgroundColor: colors.background, borderColor, borderWidth: isDelete || isTerminal ? 2 : 1 }]}>
          ...
          <View style={styles.codeActions}>
            ...
            {!codeBlockActions[key] ? (
              <View style={{ flexDirection: 'row', gap: 4, alignItems: 'center' }}>
                <TouchableOpacity
                  onPress={() => setCodeBlockActions(prev => ({ ...prev, [key]: 'refused' }))}
                ...
                <TouchableOpacity
                  onPress={() => Share.share({ message: block.code })}
              ...
              <View style={{ flexDirection: 'row', gap: 4, alignItems: 'center' }}>
                ...
                {codeBlockActions[key] === 'applied' && lastApplied && (Date.now() - lastApplied.timestamp < 60000) && (
                  <TouchableOpacity
                    onPress={() => {
                      Alert.alert('Undo?', `Revert changes to ${lastApplied.path}?`, [
                        { text: 'Cancel', style: 'cancel' },
  ...
  }, [colors, codeBlockActions, lastApplied, handleApplyCode, handleRunCommand, handleDelete, handlePreview, undoLastChange]);

  // ============== RENDER AGENT ACTIONS ==============

  const renderAgentActions = useCallback((actions: AgentAction[]) => {
    return (
      <View style={styles.actionsContainer}>
        <Text style={[styles.actionsTitle, { color: colors.textMuted }]}>Agent Actions:</Text>
        {actions.map((action, idx) => (
          ...
          >
            <View style={[styles.actionIconContainer, { backgroundColor: colors.primary + '20' }]}>
              <Ionicons
                name={
                  action.type === 'write_file' ? 'document-text' :
                  action.type === 'delete_file' ? 'trash' :
                  action.type === 'run_command' ? 'terminal' :
                  action.type === 'git_operation' ? 'git-branch' :
                  action.type === 'read_file' ? 'eye' :
                  'search'
                }
                size={16}
            ...
            <View style={styles.actionInfo}>
              ...
              <Text style={[styles.actionDesc, { color: colors.textMuted }]} numberOfLines={1}>
                {action.description}
              </Text>
            </View>
        ...
        <View style={[styles.messageContent, isUser && styles.userMessageContent]}>
          {isUser ? (
            ...
            <View style={[styles.assistantMessageBg, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              ...
              {item.actions && item.actions.length > 0 && renderAgentActions(item.actions)}
            </View>
          )}
        </View>
      </View>
    );
  }, [colors, renderCodeBlock, renderAgentActions]);
          ...
          <View style={styles.headerRight}>
            {messages.length > 0 && (
              <TouchableOpacity
                ...
                onPress={() => {
                  Alert.alert('Clear History?', 'This will delete all messages for this project.', [
                    { text: 'Cancel', style: 'cancel' },
            ...
            <TouchableOpacity
              style={[styles.projectBtn, { backgroundColor: colors.surface, borderColor: colors.border }]}
              onPress={() => setShowProjectPicker(true)}
            >
              <Ionicons name="folder" size={16} color={currentProject ? colors.success : colors.textMuted} />
              <Text style={[styles.projectBtnText, { color: colors.textSecondary }]} numberOfLines={1}>
          ...
          <BlurView intensity={80} tint={isDarkMode ? 'dark' : 'light'} style={[styles.modelPicker, { borderBottomColor: colors.border }]}>
            {availableModels.map((model) => (
              ...
              >
                <Ionicons
                  name={
                    model === 'gemini' ? 'sparkles' :
                    model === 'ollama' ? 'hardware-chip' :
                    model === 'claude' ? 'chatbubble-ellipses' :
                    'cube'
                  }
                  size={20}
                  color={selectedModel === model ? colors.primary : colors.textMuted}
                />
              ...
              <LinearGradient colors={[colors.surfaceLight, colors.surface]} style={styles.modalGradient}>
                ...
                <ScrollView style={styles.projectList}>
                  <TouchableOpacity
                    ...
                    onPress={() => { setCurrentProject(null); setShowProjectPicker(false); }}
                  >
                  ...
                  {projects.map((project) => (
                    ...
                    >
                      ...
                      <View style={styles.projectItemInfo}>
                        <Text style={[styles.projectItemName, { color: currentProject?.path === project.path ? colors.primaryLight : colors.text }]}>
                          {project.name}
                        </Text>
          ...
          <LinearGradient colors={[colors.background, isDarkMode ? '#0f0f18' : '#f1f5f9']} style={styles.codeModalContainer}>
            <SafeAreaView style={styles.codeModalContainer} edges={['top', 'bottom']}>
              <View style={[styles.codeModalHeader, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
                <LinearGradient
                  colors={LANGUAGE_COLORS[expandedCode?.language || ''] || [colors.primary, colors.primaryDark]}
                  style={styles.codeModalLangBadge}
                >
                  <Text style={styles.codeModalLangText}>{expandedCode?.language.toUpperCase()}</Text>
                </LinearGradient>
                <View style={styles.codeModalActions}>
                  <TouchableOpacity onPress={() => {
                    if (expandedCode) handleApplyCode(expandedCode.code, expandedCode.language);
...
const createStyles = (colors: any) => StyleSheet.create({
  ...
  // Agent Actions
  actionsContainer: { marginTop: 12, padding: 12, backgroundColor: colors.background, borderRadius: 12 },
  actionsTitle: { fontSize: 12, fontWeight: '700', marginBottom: 8, letterSpacing: 0.5 },
  actionCard: { flexDirection: 'row', alignItems: 'center', padding: 12, borderRadius: 10, borderWidth: 1, marginBottom: 8, gap: 10 },
  actionIconContainer: { width: 36, height: 36, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  actionInfo: { flex: 1 },
